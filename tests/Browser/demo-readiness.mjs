...
const clean = (value) => String(value ?? '').replaceAll(PASSWORD, '[MASKED]');
...
async function login(page, email) {
  ...
  await Promise.all([
    page.waitForURL(/\/(dashboard|security\/mfa\/(?:enroll|challenge))/, { timeout: 15000 }),
...
async function enroll(page) {
  ...
  await Promise.all([
    page.waitForURL(/\/security\/mfa\/recovery-codes$/),
...
async function logout(page) {
  ...
  if (await button.count()) {
    await Promise.all([page.waitForURL(/\/login$/), button.first().click()]);
  }
}
...
async function main() {
...

  {
    ...
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'guest');
    ...
    await responsive(page, '/', 'guest', 'public home'); cleanRun(); await ctx.close();
  }

  let adminMfa;
  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'system_admin');
    ...
    await Promise.all([page.waitForURL(/\/dashboard$/), page.getByRole('button', { name: 'Use recovery code' }).click()]);
    await logout(page);
    ...
    await Promise.all([page.waitForURL(/\/dashboard$/), page.getByRole('button', { name: 'Verify and continue' }).click()]);
    ...
    await logout(page); await ctx.close();
  }

  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'mayor_approver');
  ...
  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'system_admin-post-data');
    await login(page, 'admin@talibon.demo');
    await page.getByLabel('Authenticator code').fill(totp(adminMfa.secret));
    await Promise.all([page.waitForURL(/\/dashboard$/), page.getByRole('button', { name: 'Verify and continue' }).click()]);
    ...
    cleanRun(); await logout(page); await ctx.close();
  }

  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'engineering department_head');
  ...
  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'budget department_head');
  ...
  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'hr_officer');
  ...
  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'legislative_staff');
  ...
  {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 800 } });
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'employee');
...

  {
    ...
    const page = await ctx.newPage(); const cleanRun = monitor(page, 'guest-post-data');
    await page.goto(`${BASE}/`, { waitUntil: 'domcontentloaded' });
    const pub = await text(page);
    ...
    await overflow(page, 'guest post-data mobile'); cleanRun(); await ctx.close();
  }
...
main().catch(async (error) => {
  report.checks.push({ name: 'browser QA aborted', ok: false, details: clean(error?.message || error), severity: 'P1' });
  try {
    await fs.mkdir('storage/app/qa', { recursive: true });
    await fs.writeFile('storage/app/qa/demo-readiness-report.json', JSON.stringify(report, null, 2));
  } catch {}
  console.error(`DEMO_READINESS_QA_FAIL ${clean(error?.message || error)}`);
  process.exitCode = 1;
});
