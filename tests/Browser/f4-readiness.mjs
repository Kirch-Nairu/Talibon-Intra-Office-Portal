...
async function correspondence(page){
  ...
  start=runtime.length;await page.setViewportSize({width:390,height:844});r=await page.goto(`${BASE}/correspondence`);check('correspondence mobile loads',r?.status()===200);const m=page.getByRole('region',{name:'Correspondence inbox'}).getByRole('article').first();if(await m.count()){check('correspondence mobile responsibility visible',await m.getByText('Current responsibility',{exact:true}).isVisible());check('correspondence mobile action reachable',await m.getByRole('link',{name:/^Open correspondence /}).isVisible());}await noOverflow(page,'correspondence mobile');await shot(page,'correspondence-mobile-light.png');await noErrors('correspondence mobile',start);
