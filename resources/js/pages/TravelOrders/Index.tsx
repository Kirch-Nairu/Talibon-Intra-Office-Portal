    ...
    return (
        <AppLayout title="Approved Travel Orders">
            <div className="mx-auto max-w-7xl space-y-4 sm:space-y-6">
                    ...
                    <ProgressiveFilterBar
                        ...
                        primary={(
                            <label className="block">
                                <span className="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-400 sm:text-xs">Search approved orders</span>
                ...
                <section aria-label="Approved travel order registry" className="overflow-hidden rounded-xl border border-slate-200 bg-white text-slate-900 ">
                    <div className="flex flex-col gap-1 border-b border-slate-100 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                        <div className="flex items-center gap-2 text-[13px] font-bold text-slate-800 sm:text-sm"><FileCheck2 size={15} /> Approved-order registry</div>
                        <div className="text-xs text-slate-500 sm:text-xs">{travelOrders.total === 0 ? 'No matching orders' : `Showing ${travelOrders.from || 1}–${travelOrders.to || travelOrders.data.length} of ${travelOrders.total}`}</div>
                    </div>
                                ...
                                <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(460px,1fr)_auto] lg:items-start">
                                    <div className="min-w-0">
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-bold uppercase tracking-wide text-slate-600 sm:text-xs">{humanize(order.status)}</span>
                                        </div>
                                        <h2 className="mt-1.5 break-words text-[13px] font-semibold leading-5 text-slate-950 sm:text-sm">{order.purpose}</h2>
                                        <div className="mt-2 inline-flex max-w-full items-start gap-1.5 text-xs font-medium text-slate-600 sm:text-xs"><MapPin size={13} className="mt-0.5 shrink-0 text-slate-400" /><span className="break-words">{order.destination}</span></div>
                                    </div>

                                    <dl className="grid grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
                                        <div className="min-w-0">
                                            <dt className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-xs">Travel period</dt>
                                            <dd className="mt-1 flex items-start gap-1.5 break-words text-xs font-semibold text-slate-700 sm:text-xs"><CalendarDays size={12} className="mt-0.5 shrink-0 text-slate-400" />{order.travelStartDate} — {order.travelEndDate}</dd>
                                        </div>
                                        <div className="min-w-0">
                                            <dt className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-xs">Responsible office</dt>
                                            <dd className="mt-1 break-words text-xs font-semibold text-slate-700 sm:text-xs">{officeLabel(order.office)}</dd>
                                        </div>
                                        <div className="min-w-0">
                                            <dt className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-xs">Personnel issued</dt>
                                            <dd className="mt-1 flex items-center gap-1.5 text-xs font-semibold text-slate-700 sm:text-xs"><Users size={12} className="shrink-0 text-slate-400" />{order.issuedToCount} {order.issuedToCount === 1 ? 'person' : 'personnel'}</dd>
                                        </div>
                                        <div className="min-w-0">
                                            <dt className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-xs">Issued</dt>
                                            <dd className="mt-1 text-xs font-semibold text-slate-700 sm:text-xs">{order.issuanceDate}</dd>
                                        </div>
                                    </dl>

                                    <Link href={order.detailUrl} aria-label={`Open travel order ${order.referenceNumber}`} className="inline-flex w-fit items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 sm:text-xs lg:justify-self-end">
                    ...
                    {travelOrders.last_page > 1 && <div className="flex items-center justify-between gap-3 border-t border-slate-100 px-4 py-3 sm:px-5"><Link href={travelOrders.prev_page_url || '#'} preserveScroll className={`rounded-lg border px-3 py-2 text-xs font-semibold sm:text-xs ${travelOrders.prev_page_url ? 'border-slate-300 text-slate-700 hover:bg-slate-50' : 'pointer-events-none border-slate-100 text-slate-300'}`}>Previous</Link><div className="text-xs text-slate-500 sm:text-xs">Page {travelOrders.current_page} of {travelOrders.last_page}</div><Link href={travelOrders.next_page_url || '#'} preserveScroll className={`rounded-lg border px-3 py-2 text-xs font-semibold sm:text-xs ${travelOrders.next_page_url ? 'border-slate-300 text-slate-700 hover:bg-slate-50' : 'pointer-events-none border-slate-100 text-slate-300'}`}>Next</Link></div>}
                </section>
            </div>
        </AppLayout>
    );
