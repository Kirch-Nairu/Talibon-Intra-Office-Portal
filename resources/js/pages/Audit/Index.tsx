            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-700">
                {events.map(event => <article key={event.id} className="grid min-w-0 gap-3 px-4 py-4 sm:px-5 lg:grid-cols-[minmax(0,1.6fr)_minmax(180px,0.8fr)_150px] lg:items-start">
                    <div className="min-w-0">
                        ...
                        <div className="mt-2 break-all font-mono text-xs text-slate-400">Event code: {event.action}</div>
                    </div>
                    ...
                    <div className="text-xs text-slate-500 dark:text-slate-400 lg:text-right"><div className="font-bold uppercase tracking-wide text-slate-400">Recorded</div><time className="mt-0.5 block break-words" dateTime={event.created_at}>{new Date(event.created_at).toLocaleString()}</time></div>
                </article>)}
