        ...
        <AppLayout title={tx.reference_no}>
            <div className="mx-auto max-w-6xl space-y-4 sm:space-y-6">
                        ...
                        {permissions.canAssign && (
                            <div className="mt-3 rounded-xl border border-blue-100 bg-white p-3 sm:mt-4 sm:p-4">
                                ...
                                <div className="mt-2 flex gap-2">
                                    <select value={form.data.assigned_employee_id} onChange={(e) => form.setData('assigned_employee_id', Number(e.target.value))} className="min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-2.5 py-2 text-[11px] sm:rounded-xl sm:px-3 sm:py-2.5 sm:text-sm"><option value="">Choose employee…</option>{assignableEmployees.map((employee) => <option key={employee.id} value={employee.id}>{employee.full_name || employee.employee_number} · {employee.position_title}</option>)}</select>
                        ...
                        {permissions.canTransition && (
                            <div className="mt-3 flex flex-wrap gap-2 sm:mt-4 sm:gap-3">
                                ...
                                <div className="flex min-w-0 basis-full gap-2 sm:min-w-[280px] sm:flex-1 sm:basis-auto">
                                    <select value={form.data.target_department_id} onChange={(e) => form.setData('target_department_id', Number(e.target.value))} className="min-w-0 flex-1 rounded-lg border border-blue-200 bg-white px-2.5 py-2 text-[12px] sm:rounded-xl sm:px-3 sm:py-2.5 sm:text-sm"><option value="">Forward to department…</option>{departments.filter((d) => d.id !== tx.current_department.id).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select>
                ...
                <section className="rounded-2xl border border-slate-200 bg-white shadow-sm sm:rounded-3xl">
                    ...
                    <div className="divide-y divide-slate-100">{tx.events.map((event) => <div key={event.id} className="grid gap-2.5 px-4 py-3.5 sm:gap-4 sm:px-6 sm:py-5 md:grid-cols-[32px_1fr_180px]"><div className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-50 text-blue-800 sm:h-8 sm:w-8"><ArrowRight size={14} className="sm:h-[15px] sm:w-[15px]" /></div><div><div className="text-[13px] font-semibold capitalize text-slate-950 sm:text-base">{event.action.replaceAll('_', ' ')}</div><div className="mt-1 text-[11px] text-slate-600 sm:text-sm">{event.from_department?.short_name || event.from_department?.name || '—'} → {event.to_department?.short_name || event.to_department?.name || '—'}</div>{event.remarks && <div className="mt-2 rounded-lg bg-slate-50 px-2.5 py-1.5 text-[11px] text-slate-600 sm:rounded-xl sm:px-3 sm:py-2 sm:text-sm">{event.remarks}</div>}<div className="mt-2 text-[9px] text-slate-400 sm:text-xs">By {event.actor.name}</div></div><div className="text-[9px] text-slate-500 sm:text-xs md:text-right">{new Date(event.created_at).toLocaleString()}</div></div>)}</div>
                </section>
            </div>
        </AppLayout>
    );
