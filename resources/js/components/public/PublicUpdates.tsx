...
export default function PublicUpdates({ content }: { content: PublicContent }) {
    ...
    return <div className="grid items-start gap-4 @min-[900px]:grid-cols-2">
        ...
        <PublicPanel id="projects" title="Projects & Programs" icon={Building2}>
            <div className="mb-2 text-xs font-semibold text-amber-800 dark:text-amber-300">Sample content</div>
            <div className="space-y-2">{content.projects.map((item) => <article key={item.title} className="border-b border-slate-200 py-3 last:border-0 dark:border-slate-700">
                <div className="text-xs font-semibold text-slate-500 dark:text-slate-400">{item.tag}</div>
                <h3 className="mt-1 text-sm font-semibold leading-5">{item.title}</h3><p className="mt-1 text-sm leading-5 text-slate-600 dark:text-slate-400">{item.summary}</p>
            </article>)}</div>
        ...
        <PublicPanel id="transparency" title="Transparency & Public Documents" icon={FileText}>
            ...
            <div className="divide-y divide-slate-200 dark:divide-slate-700">{content.transparency.map((item) => <article key={item.label} className="flex items-start gap-2 py-3">
                ...
                <div><h3 className="text-xs font-semibold">{item.label}</h3><div className="mt-1 text-sm font-medium text-blue-700 dark:text-blue-300">{item.value}</div><p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">{item.note}</p></div>
            </article>)}</div>
            <p className="mt-2 border-t border-slate-200 pt-3 text-xs leading-5 text-slate-500 dark:border-slate-700 dark:text-slate-400">Public document library preview. Downloadable documents will appear when published.</p>
        </PublicPanel>
    </div>;
}
