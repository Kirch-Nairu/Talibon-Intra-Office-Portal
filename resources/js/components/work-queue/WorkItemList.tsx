...
export default function WorkItemList({ records, title }: { records: Paginator; title: string }) {
    return (
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white text-slate-900 " aria-label={title}>
            <div className="flex flex-col gap-1 border-b border-slate-100 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                <div className="text-[13px] font-bold text-slate-900 sm:text-sm">{title}</div>
            ...
            <div className="hidden grid-cols-[minmax(250px,1.45fr)_145px_155px_145px_minmax(190px,1fr)_88px] gap-4 border-b border-slate-100 px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-500 xl:grid">
                        ...
                        <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(250px,1.45fr)_145px_155px_145px_minmax(190px,1fr)_88px] xl:items-center">
                            <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                                    ...
                                    <span className="text-xs text-slate-500">{humanize(item.transactionType)}</span>
                                </div>
                                ...
                                <div className="mt-2 flex flex-wrap gap-1.5" aria-label="Work item state">
                                    ...
                                    <span className="border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-bold uppercase text-slate-700 sm:text-xs">
                                    ...
                                    {item.requiresAction ? (
                                        <span className="inline-flex items-center gap-1 border border-blue-200 bg-blue-50 px-2 py-1 text-xs font-bold uppercase text-blue-800 sm:text-xs">
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:contents">
                                <div className="min-w-0">
                                    <div className="text-xs font-bold uppercase tracking-wide text-slate-400 xl:hidden">Accountability</div>
                                    <div className="mt-1 flex items-start gap-1.5 text-xs font-semibold leading-4 text-slate-700 sm:text-xs">
                                    ...
                                    <div className="mt-1 text-xs text-slate-500">From {item.originOffice?.shortName || item.originOffice?.name || 'Unknown'}</div>
                                </div>

                                <div className="min-w-0">
                                    <div className="text-xs font-bold uppercase tracking-wide text-slate-400 xl:hidden">Assignment</div>
                                    <div className="mt-1 text-xs font-semibold leading-4 text-slate-700 sm:text-xs">{item.assignedEmployee?.name || 'Unassigned'}</div>
                                    {item.assignedEmployee?.position ? <div className="mt-1 text-xs leading-4 text-slate-500">{item.assignedEmployee.position}</div> : null}
                                </div>

                                <div className="min-w-0">
                                    <div className="text-xs font-bold uppercase tracking-wide text-slate-400 xl:hidden">Due</div>
                                    <div className="mt-1 flex items-center gap-1.5 text-xs font-semibold text-slate-700 sm:text-xs">
                                    ...
                                    <div className="mt-1 text-xs text-slate-500">Updated {formatDate(item.updatedAt)}</div>
                                </div>
                            </div>

                            <div className="border-l-2 border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold leading-4 text-slate-700 sm:text-xs xl:min-h-12">
                                <div className="mb-0.5 text-xs font-bold uppercase tracking-wide text-slate-400 xl:hidden">Next action</div>
                                {item.expectedAction}
                            </div>
            ...
            {records.last_page > 1 ? (
                <div className="flex items-center justify-between gap-3 border-t border-slate-100 px-4 py-3 sm:px-5">
                    <Link href={records.prev_page_url || '#'} preserveScroll className={`border px-3 py-2 text-xs font-semibold sm:text-xs ${records.prev_page_url ? 'border-slate-300 text-slate-700 hover:bg-slate-50' : 'pointer-events-none border-slate-100 text-slate-300'}`}>Previous</Link>
                    <div className="text-xs text-slate-500 sm:text-xs">Page {records.current_page} of {records.last_page}</div>
                    <Link href={records.next_page_url || '#'} preserveScroll className={`border px-3 py-2 text-xs font-semibold sm:text-xs ${records.next_page_url ? 'border-slate-300 text-slate-700 hover:bg-slate-50' : 'pointer-events-none border-slate-100 text-slate-300'}`}>Next</Link>
                </div>
            ) : null}
        </section>
    );
