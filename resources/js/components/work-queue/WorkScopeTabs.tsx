...
export default function WorkScopeTabs({ groups, currentView, onSelect }: {
    ...
    return (
        <section
            className="space-y-4 rounded-xl border border-slate-200 bg-white p-3 text-slate-900  sm:p-4"
            aria-label="Work queues"
        >
            {groups.map((group) => (
                <div key={group.key}>
                    <div className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-500 sm:text-xs">
                        {group.label}
                    </div>
                        ...
                        {group.views.map((view) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className={`mt-0.5 text-xs sm:text-xs ${active ? 'text-blue-700' : 'text-slate-500'}`}>
