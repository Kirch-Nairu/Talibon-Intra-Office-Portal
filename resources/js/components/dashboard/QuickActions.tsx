    ...
    return <section className="municipal-panel p-4" aria-labelledby="dashboard-quick-actions">
        ...
        <div className={`mt-3 grid gap-2 ${columns}`}>
            {actions.map((action) => {
                ...
                return <Link key={action.url} href={action.url} className="flex min-h-20 min-w-0 items-center gap-3 rounded-lg border border-slate-200 px-4 py-3 transition-colors duration-150 hover:border-[#1769aa] hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">
                    <Icon size={22} className="shrink-0 text-[#1769aa] dark:text-blue-300" strokeWidth={1.8} aria-hidden="true" />
