    ...
    return (
        <section className="space-y-3.5" aria-labelledby="dashboard-office-overview">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <div className="text-xs font-bold  text-blue-700 dark:text-blue-300 sm:text-xs">Office accountability</div>
                    ...
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-500 dark:text-slate-400 sm:text-xs">Staff assignments and work requiring follow-up.</p>
                </div>
            ...
            <div className="grid gap-3 @min-[900px]:grid-cols-[1.2fr_0.8fr]">
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-700">
                        {overview.staffWorkload.map((row) => (
                            <article key={`${row.employee}-${row.position || ''}`} className="grid gap-2 px-4 py-3 @min-[600px]:grid-cols-[minmax(0,1fr)_75px_75px_90px] sm:items-center sm:px-5">
                                <div className="min-w-0"><div className="break-words text-sm font-semibold text-slate-800 dark:text-slate-100 sm:text-xs">{row.employee}</div><div className="mt-0.5 break-words text-xs text-slate-500 dark:text-slate-400">{row.position || 'Position not recorded'}</div></div>
                        ...
                        {overview.staffWorkload.length === 0 ? <div className="px-5 py-7 text-center text-sm text-slate-500 dark:text-slate-500 dark:text-slate-400">No active assigned office work.</div> : null}
                    </div>
                </div>
                ...
                <div className="overflow-hidden rounded-xl border border-slate-200 bg-white  dark:border-slate-700 dark:bg-[#142236]">
                    ...
                    <div className="divide-y divide-slate-100 px-4 dark:divide-slate-700 sm:px-5">
                        ...
                        {overview.statusOverview.length === 0 ? <div className="py-7 text-center text-sm text-slate-500 dark:text-slate-500 dark:text-slate-400">No active office status rows.</div> : null}
                    </div>
                </div>
            </div>
