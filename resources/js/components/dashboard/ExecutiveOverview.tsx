        ...
        <section className="space-y-3.5" aria-labelledby="dashboard-executive-overview">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-[10px] text-slate-500 dark:text-slate-400 sm:text-xs">Office workload and unresolved transactions for municipal follow-up.</p>
                </div>
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-[#142236]">
                ...
                <div className="hidden grid-cols-[minmax(0,1.5fr)_repeat(4,82px)] gap-3 border-b border-slate-100 bg-slate-50 px-5 py-2 text-[9px] font-bold uppercase tracking-wide text-slate-400 dark:border-slate-700 dark:bg-slate-900/40 @min-[700px]:grid">
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                    {overview.departmentWorkload.map((office) => (
                        <article key={office.id} className="grid gap-2 px-4 py-3 sm:px-5 @min-[700px]:grid-cols-[minmax(0,1.5fr)_repeat(4,82px)] md:items-center">
                            ...
                            <div className="text-[10px] text-slate-600 dark:text-slate-300 sm:text-xs"><span className="font-bold text-slate-950 dark:text-slate-100">{office.active}</span> <span className="@min-[700px]:hidden">active</span></div>
                            <div className={`text-[10px] sm:text-xs ${office.unassigned > 0 ? 'font-semibold text-amber-700 dark:text-amber-300' : 'text-slate-600 dark:text-slate-300'}`}>{office.unassigned} <span className="@min-[700px]:hidden">unassigned</span></div>
                            <div className="text-[10px] text-amber-700 dark:text-amber-300 sm:text-xs">{office.dueSoon} <span className="@min-[700px]:hidden">due soon</span></div>
