        ...
        <section className="space-y-3.5" aria-labelledby="dashboard-executive-overview">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-500 dark:text-slate-400 sm:text-xs">Office workload and unresolved transactions for municipal follow-up.</p>
                </div>
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                    {overview.departmentWorkload.map((office) => (
                        <article key={office.id} className="grid gap-2 px-4 py-3 sm:px-5 @min-[700px]:grid-cols-[minmax(0,1.5fr)_repeat(4,82px)] md:items-center">
                            <div className="min-w-0"><div className="break-words text-sm font-semibold text-slate-800 dark:text-slate-100 sm:text-xs">{office.shortName || office.name}</div><div className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{office.code}</div></div>
                            ...
                            <div className={`inline-flex items-center gap-1 text-xs sm:text-xs ${office.overdue > 0 ? 'font-semibold text-rose-700 dark:text-rose-300' : 'text-slate-500 dark:text-slate-500 dark:text-slate-400'}`}><AlertTriangle size={11} aria-hidden="true" /> {office.overdue} overdue</div>
                        </article>
                    ))}
                    {overview.departmentWorkload.length === 0 ? <div className="px-5 py-7 text-center text-sm text-slate-500 dark:text-slate-500 dark:text-slate-400">No active municipal workload.</div> : null}
                </div>
            </div>
